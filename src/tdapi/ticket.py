...
class TDBaseTicket(tdapi.obj.TDObject):
    ...
    def _ensure_single_query(self):
        if self._single_queried is False:
            self.td_struct = tdapi.TD_CONNECTION.json_request(method="get", url_stem=self.url())
    ...
    def feed(self):
        ...
        """
        return tdapi.TD_CONNECTION.json_request_roller(method="get", url_stem=self.url() + "/feed")
    ...
    def tasks(self):
        ...
        """
        return tdapi.TD_CONNECTION.json_request_roller(method="get", url_stem=self.url() + "/tasks")
    ...
    def add_to_feed(self, status_id, is_private, comments, notify):
        ...
        updated_item = tdapi.TD_CONNECTION.json_request(
            method="post", url_stem=feed_update_url, data=data
        )
        return updated_item

    def notify_values(self):
        return [x["Value"] for x in self.single_query_get("Notify")]
