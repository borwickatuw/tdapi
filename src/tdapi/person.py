...
class TDPersonManager(tdapi.obj.TDObjectManager):
    ...
    def userlist(self, active=None, employee=None, user_type=None):
        ...
        return [
            TDPerson(td_struct)
            for td_struct in tdapi.TD_CONNECTION.json_request_roller(
                method="get", url_stem=userlist_url
            )
        ]
...
class TDPerson(tdapi.obj.TDObject):
    ...
    def _ensure_single_query(self):
        if self._single_queried is False:
            self.td_struct = tdapi.TD_CONNECTION.json_request(
                method="get", url_stem=self.person_url()
            )
    ...
    def update(self, update_data):
        ...
        tdapi.TD_CONNECTION.request(method="post", url_stem=self.person_url(), data=update_data)
    ...
    def del_applications(self, app_list):
        all_apps = [x for x in self.td_struct["Applications"] if x not in app_list]
        return self.update({"Applications": all_apps})
    ...
    def new(cls, update_data):
        ...
        tdapi.TD_CONNECTION.request(method="post", url_stem="people", data=update_data)
